...
// ---------- qrStuck (watchdog) ----------
test('qrStuck: fresh QR = not stuck', () => {
  const now = 1000000;
  assert.strictEqual(qrStuck(now - 30000, now, 120000), false);
});

test('qrStuck: no QR for longer than grace = stuck', () => {
  const now = 1000000;
  assert.strictEqual(qrStuck(now - 130000, now, 120000), true);
});

test('qrStuck: never got a QR = not yet stuck (give it time)', () => {
  assert.strictEqual(qrStuck(null, 1000000, 120000), false);
  assert.strictEqual(qrStuck(0, 1000000, 120000), false);
});

test('qrStuck: stops watching once authenticated', () => {
  // QRs stop after linking by design — must never trigger while authenticated
  assert.strictEqual(qrStuck(1000, 1000000, 120000, true), false);
});

test('qrStuck: exact grace boundary not stuck (strictly greater)', () => {
  const now = 1000000;
  assert.strictEqual(qrStuck(now - 120000, now, 120000), false);
});
